...
from my_dataset_reduction import phl_selection, phl_selection_from_scores, phl_scores, get_max_distance, \
                                 get_super_outliers, get_mean_neighbors
...
metrics = ['reduction_ratio', 'representativeness', 'accuracy', 'f1', 'training_time', 'reduction_time', 'super-outliers']

# PHL Hyperparameter experiments
phl_results = pd.DataFrame(columns=['model', 'reduction_method', 'mode', 'dimensions', 'max_dimension', 'percentage', 'delta'] + metrics) # Store results of PHL hyperparameter experiments
super_outliers_df = pd.DataFrame(columns=['delta', 'super_outliers', 'mean_neighbors']) # Store super outliers and mean neighbors for each delta
...
dimensions = [(0, 'restrictedDim'), (1, 'restrictedDim'), (2, 'multiDim')] # (1, 'restrictedDim') # (2, 'multiDim')] # if not PROFILING else [(1, 'restrictedDim')]
...
for delta in tqdm.tqdm(deltas, desc="Delta"):
    for dimension, scoring_version in tqdm.tqdm(dimensions, desc="Dimension", leave=False):
